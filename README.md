...
- **前端动态渲染加固**：通知渠道列表、AI 回复测试结果和版本信息弹窗中的接口数据已统一转义后再写入页面，降低动态 `innerHTML` 带来的 XSS 风险。
- **回归测试覆盖**：新增静态 JavaScript 测试，锁定关键动态渲染点必须使用 `escapeHtml`，避免后续改动重新引入未转义插入。
- **页面 smoke 校验**：保留前台 HTTP smoke 和静态文案编码测试，确保安全修复不影响登录页、管理页和脚本加载。
